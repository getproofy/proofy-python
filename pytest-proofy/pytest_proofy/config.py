...
def resolve_options(config: pytest.Config) -> ProofyConfig:
    ...
    return ProofyConfig(
        ...
        project_id=get_option("proofy_project_id", "PROOFY_PROJECT_ID", "proofy_project_id", type_func=int),
        
        batch_size=get_option("proofy_batch_size", "PROOFY_BATCH_SIZE", "proofy_batch_size", 10, int),
        
        output_dir=get_option("proofy_output_dir", "PROOFY_OUTPUT_DIR", "proofy_output_dir", "proofy-artifacts"),
        always_backup=get_option("proofy_always_backup", "PROOFY_ALWAYS_BACKUP", "proofy_always_backup", False, bool),
        ...
        enable_attachments=not get_option("proofy_disable_attachments", "PROOFY_DISABLE_ATTACHMENTS", "proofy_disable_attachments", False, bool),
        enable_hooks=not get_option("proofy_disable_hooks", "PROOFY_DISABLE_HOOKS", "proofy_disable_hooks", False, bool),
        
        timeout_s=get_option("proofy_timeout", "PROOFY_TIMEOUT", "proofy_timeout", 30.0, float),
        max_retries=get_option("proofy_max_retries", "PROOFY_MAX_RETRIES", "proofy_max_retries", 3, int),
    )
