"""Unified attachment preparation service.
...
class AttachmentService:
    """Unified service for preparing attachments with caching, MIME detection, and hashing.

    This service centralizes all attachment preparation logic in one place,
    eliminating duplication and providing a consistent interface.
    """

    def __init__(self, config: ProofyConfig) -> None:
        """Initialize attachment service with configuration.

        Args:
            config: Proofy configuration containing mode, caching settings, etc.
        """
        self.config = config
    ...
    def prepare_attachment(
        ...
        *,
        ...
        - Caching based on mode and config
        - MIME type detection
        ...
        should_cache = should_cache_for_mode(self.config.mode)
    ...
    def prepare_traceback(self, text: str, base_name: str) -> PreparedAttachment:
